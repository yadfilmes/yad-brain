...
| nota | citada por |
|---|---|
| `aces` | `ampas` (see_also), `arri-wide-gamut-4` (conforms_to_pipeline), `braw` (conforms_to_pipeline), `ecossistema-blackmagic` (see_also), `log-c4` (conforms_to_pipeline), `rec-709` (see_also), `s-gamut3-cine` (conforms_to_pipeline), `s-log3` (conforms_to_pipeline) |
...
| `arri-wide-gamut-4` | `log-c4` (paired_gamut) |
| `arriraw` | `alexa-35` (records_codec), `braw` (see_also), `log-c4` (see_also), `x-ocn` (see_also) |
| `bat--b-mount` | `alexa-35` (uses_battery_mount) |
| `blackmagic-design` | `braw` (made_by), `ecossistema-blackmagic` (made_by), `pyxis-6k` (made_by) |
| `braw` | `aces` (see_also), `arriraw` (see_also), `blackmagic-design` (see_also), `ecossistema-blackmagic` (see_also), `prores` (see_also), `pyxis-6k` (records_codec), `x-ocn` (see_also) |
...
| `venice-2` | `00-indice-mestre` (see_also), `alexa-35` (competes_with), `filtro-nd` (see_also), `pyxis-6k` (budget_alternative_to), `pyxis-6k` (competes_with), `s-log3` (see_also), `sony` (see_also) |
| `x-ocn` | `arriraw` (see_also), `braw` (see_also), `s-log3` (see_also), `venice-2` (records_codec) |
| `xavc` | `venice-2` (records_codec) |
